...
export default function ServicesPage() {
  ...
  const yParallax = shouldReduceMotion
    ? 0
    : useTransform(scrollY, [0, 500], [0, -120]);

  /* Magnetic interaction (refined) */
  const useMagnetic = () => {
    ...
    return {
      rotate,
      handleMouseMove,
      resetRotate: () => setRotate({ x: 0, y: 0 }),
    };
  };

  const MagneticCard = ({ service }: { service: any }) => {
    const { rotate, handleMouseMove, resetRotate } = useMagnetic();
      ...
      <Link href={service.route}>
        ...
        >
          ...
          <div className="p-6 space-y-4">
            ...
            <span className="inline-block mt-2 text-sm text-blue-400 opacity-0 group-hover:opacity-100 transition">
              View Service →
            </span>
          </div>
        </motion.div>
  ...
  return (
    <main className="bg-linear-to-br from-gray-900 via-black to-gray-800 text-white min-h-screen">
      ...
      {/* CTA */}
      <section className="py-24 bg-linear-to-tr from-gray-900 via-black to-gray-800 text-white text-center px-6">
        <motion.h2
