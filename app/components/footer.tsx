...
export default async function Footer() {
  ...
  const phoneDisplay = getSetting("contact_phone_display");
  const phoneRaw = getSetting("contact_phone_raw");
              ...
              <div className="flex items-center gap-4 mt-6">
                {socials.map((s) => (
                  <a
                    ...
                    className="text-white/25 hover:text-[#24eda2] transition-colors"
                  >
                    {socialIcons[s.platform] ?? (
                      <span className="text-[12px]">{s.platform}</span>
                    )}
                  </a>
                ))}
              </div>
